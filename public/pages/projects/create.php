...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Se não houver erros, criar o projeto
    if (empty($errors)) {
        try {
            $stmt = $db->prepare("
                INSERT INTO projects (name, description, enterprise_id, status, created_at)
                VALUES (?, ?, ?, ?, NOW())
            ");
            $stmt->execute([$name, $description, $enterprise_id, $status]);
                        ...
                        <div class="form-group">
                            ...
                            <select id="enterprise_id" name="enterprise_id" required>
                                ...
                                <?php foreach ($enterprises as $enterprise): ?>
                                    <option value="<?php echo $enterprise['id']; ?>" <?php echo isset($_POST['enterprise_id']) && $_POST['enterprise_id'] == $enterprise['id'] ? 'selected' : ''; ?>>
